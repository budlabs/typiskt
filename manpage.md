...
on which mode you start **typiskt** in. No matter
which mode you can press <kbd>Escape</kbd> to
restart the test, and <kbd>Ctrl</kbd>+<kbd>c</kbd>
to quit.
...
get recorded. The default corpus is *english*, but
...
argument to `--book`) and will be displayed in
...
(**words**) mode. Defaults to *english*. This
value can also be set in
`TYPISKT_CONFIG_DIR/config` or with the
...
**TYPISKT_CONFIG_DIR/exercises/EXERCISE** for
...
defaults to: $XDG_CONFIG_HOME/typiskt
